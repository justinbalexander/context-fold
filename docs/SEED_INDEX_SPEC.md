...
It is written as a spec rather than an internal detail, so that other emitters
...
and `src/core/index/seed-index.ts` is one implementation of it.
...
Location is emitter-owned, and the Pi extension writes
...
enumerating the directory, because emitters may keep bookkeeping files alongside
the data. The Pi extension writes one such file, `.alive`, a liveness heartbeat its
...
- `spans`: the recovery pointers. Each names the durable artifact holding the
  ...
  ground-truth store for the span, and `code` is the in-context recall handle when
  the emitter has one.
  ...
  then slice. Another emitter might point at a plain session log, where the offsets
...
- A `/recall`-style skill treats the union of records as the session's index:
  ...
  whole spans (or scripted scans over them) rather than one pointer at a time.
- Journal synthesis may read all fields but must treat any narrative it adds
  as its own, because the index itself is ground truth extracted verbatim.
- Records are advisory for context reconstruction, so consumers verify claims
