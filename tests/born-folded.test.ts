...
describe("born-folded blocks (L0 gate)", () => {
	...
	it("budget counts pointer weight; ranking sees full weight", () => {
		...
		const withGate = new ContextFoldEngine(new FoldLadderPolicy(), { tailTarget: 100 }, gate);
		const withoutGate = new ContextFoldEngine(new FoldLadderPolicy(), { tailTarget: 100 }, new MapGateRegistry());
	...
	it("is terminal: the policy never re-folds a born-folded block", () => {
		...
		const engine = new ContextFoldEngine(new FoldLadderPolicy(), { tailTarget: 200, defaultContextWindow: 8_000 }, gate);
