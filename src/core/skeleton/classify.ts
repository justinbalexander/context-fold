 ...
 *
 * Ported verbatim from Accordion `conductors/code-skeleton/classify.ts` (pinned commit 0c22434).
 * The only change is provenance: the `ViewBlock` import path (`../contract`) is unchanged — it
 * resolves to this port's `src/core/contract.ts`, whose `ViewBlock` shape is the same.
 */
...
function stripTrailingTruncation(text: string): string {
	// Manual trailing-filler trim + short anchored regex. The obvious one-regex form
	// (`/[\s.…]*[([]?\s*truncated\s*[)\]]?\s*$/`) backtracks catastrophically (~O(n³)) on a long
	// trailing whitespace/dot run — a blank-padded tool dump measurably hangs the fold pass.
	let end = text.length;
	while (end > 0 && /[\s.…]/.test(text[end - 1])) end--;
	const trimmed = text.slice(0, end);
	// Only the last few chars can hold the note; test a bounded window, never the full text.
	const window = trimmed.slice(-24);
	const m = /[([]?\s*truncated\s*[)\]]?$/i.exec(window);
	if (!m) return text;
	const cut = trimmed.length - m[0].length;
	// Require the note to sit at a word boundary (start of text, or after whitespace/punct).
	if (cut > 0 && /[\w]/.test(trimmed[cut - 1])) return text;
	let noteStart = cut;
	while (noteStart > 0 && /[\s.…]/.test(trimmed[noteStart - 1])) noteStart--;
	return trimmed.slice(0, noteStart);
}
