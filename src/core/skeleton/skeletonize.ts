 ...
 * bodied implementation is elided. Risk-flag body lines (TODO/FIXME, throw/raise/panic!) are
 * retained IN PLACE inside elided bodies (Phase 6) — the same guarantee the L2 trim gives prose:
 * a buried failure signal never vanishes into an elision marker.
 *
 * Ported from Accordion `conductors/code-skeleton/skeletonize.ts` (pinned commit 0c22434);
 * locally hardened since (mask backfill invariant, Rust lifetimes, semicolon-less depth desync,
 * risk-line retention). Still self-contained: imports nothing. The risk detector here is a
 * CODE-risk detector, deliberately separate from the ledger's tool-output detector — the
...
export interface MaskOpts {
  ...
  triple?: boolean;
  /** Rust lifetimes: a `'` followed by an identifier (`'static`, `<'a>`) is code, not a string. */
  lifetime?: boolean;
}
...
export function maskSource(src: string, opts: MaskOpts): string {
  ...
  while (i < n) {
    ...
    if (c === '"' || c === "'") {
      // Rust lifetimes: `'static`, `<'a>` — a `'` that does not close within 1 escaped/plain char
      // is a lifetime marker, not a char literal. Treat it as ordinary code so the mask doesn't
      // blank real structure between arbitrary apostrophe pairs.
      if (c === "'" && opts.lifetime) {
        const isCharLit = (c2 === "\\" && src[i + 3] === "'") || (c2 !== "'" && c2 !== "" && src[i + 2] === "'");
        if (!isCharLit) {
          copy(i);
          i++;
          continue;
      ...
      const closed = j < n; // found the closing quote before EOF
      const end = closed ? j + 1 : n;
      copy(i); // keep the opening quote
      blank(i + 1, closed ? end - 1 : end); // unterminated → blank through EOF (no index hole)
      if (closed) copy(end - 1); // keep closing quote
      i = end;
      continue;
    }
  ...
  // INVARIANT BACKSTOP: every index must be filled — an array hole would silently shorten the
  // join and desync every parallel mask[i]/orig[i] walk downstream. Backfill defensively.
...
// ----------------------------------------------------------------------------------------
// Risk-flag body lines — retained in place when a body is elided.
// ----------------------------------------------------------------------------------------

/** Max risk lines retained per elided body (bounds a validation-heavy body full of throws). */
const MAX_BODY_RISK_LINES = 4;
/** Per-line clip for a retained risk line (bounds one pathological line). */
const BODY_RISK_LINE_CLIP = 200;

/** Comment-borne markers — matched on the ORIGINAL line (comment content is blanked in the mask). */
const RISK_COMMENT_RE = /\b(?:TODO|FIXME|XXX|HACK)\b/;
/**
 * Code-borne failure signals — matched on the MASK so string contents can't trigger
 * (`"never throw"` masks to blanks). The leading guard class rejects member access (`.throw`)
 * and identifier tails (`rethrow`).
 */
const RISK_CODE_RE = /(?:^|[^.\w])(?:throw|raise)\b|\b(?:panic|todo|unimplemented|unreachable)!/;

function isRiskBodyLine(origLine: string, maskLine: string): boolean {
  if (origLine.length > 2000) return false; // a wall of minified/generated text is not a signal line
  return RISK_COMMENT_RE.test(origLine) || RISK_CODE_RE.test(maskLine);
}

function clipRiskLine(line: string): string {
  return line.length > BODY_RISK_LINE_CLIP ? line.slice(0, BODY_RISK_LINE_CLIP - 1).trimEnd() + ELL : line;
}
...
function looksCallable(maskLine: string): boolean {
  ...
  // Anchored at line start (modifiers allowed) so a mid-line call expression doesn't match.
  if (/^(?:(?:public|private|protected|static|abstract|override|readonly|async|export|default)\s+)*[A-Za-z_$][\w$]*\s*\([^;]*\)\s*(?::[^={]*)?\{?\s*$/.test(t)) {
...
function skeletonizeBrace(src: string, lang: Lang): SkeletonResult {
  ...
  let elideStartIdx = 0;
  let pendingHead = "";
  /** Risk-flag body lines collected while eliding (retained in place at the body close). */
  let bodyRisk: { idx: number; line: string }[] = [];

  // Emit an elided body: the held signature, any retained risk lines interleaved with gap
  // markers, and the close. `endIdx` is the index of the close line (or orig.length when the
  // body ran to EOF unterminated). Risk-free bodies keep the compact one-line collapse.
  // Retention affects OUTPUT only — depth tracking never sees it, so there is no desync surface.
  const flushElidedBody = (endIdx: number, afterClose: string, closeIndent: string): void => {
    const nLines = endIdx - elideStartIdx; // body lines collapsed (excludes the close line)
    if (bodyRisk.length === 0) {
      ...
      kept += 1;
      let prev = elideStartIdx - 1;
      for (const r of bodyRisk) {
        const gap = r.idx - prev - 1;
        if (gap > 0) {
          out.push(`${indentStr(r.line)}${ELL_MARK(gap)}`);
          kept += 1;
        }
        out.push(clipRiskLine(r.line));
        kept += 1;
        prev = r.idx;
      }
      const tailGap = endIdx - prev - 1;
      out.push(`${closeIndent}${tailGap > 0 ? `${ELL_MARK(tailGap)} ` : ""}}${afterClose}`);
      kept += 1;
    }
    elided += nLines - bodyRisk.length; // retained risk lines are kept, not elided
    eliding = false;
    pendingHead = "";
    bodyRisk = [];
  };
  ...
  for (let i = 0; i < orig.length; i++) {
    ...
    if (eliding) {
      ...
      if (depth <= elideTargetDepth) {
        ...
        flushElidedBody(i, afterClose, indentStr(line));
        // depth already updated; continue to next line.
        continue;
      }
      // Still inside the body: elision is accounted once, at the flush (a per-line count here
      // DOUBLE-counted every brace body — `6L → 1L · 8 elided` on a 4-line body).
      if (bodyRisk.length < MAX_BODY_RISK_LINES && !isBlank(line) && isRiskBodyLine(line, m)) {
        bodyRisk.push({ idx: i, line });
      }
      continue;
    }
    ...
    if (callableHere) {
      ...
      // A `}` on the current line means the statement ended without a body (semicolon-less
      // style: `log(x)` then the enclosing block closes) — stop, or the consumed close brace
      // would desync the depth counter and silently swallow later structure.
      while (
        !hasBrace &&
        openIdx + 1 < orig.length &&
        !/[;{]\s*$/.test(mask[openIdx].trimEnd()) &&
        !mask[openIdx].includes(";") &&
        !mask[openIdx].includes("}")
      ) {
      ...
      if (restMask && depth + restDelta <= elideTargetDepth) {
        ...
        // (Balanced one-liners like `f() { throw x; }` never reach here — the afterOpen check
        // above keeps them whole, failure signals included.)
  ...
  // (and any retained risk lines) so the signature survives.
  if (eliding) {
    flushElidedBody(orig.length, "", "");
  }
...
function skeletonizePython(src: string): SkeletonResult {
  ...
  while (i < orig.length) {
    ...
    if (isDef) {
      ...
      // Retain risk-flag body lines in place (raise/TODO/FIXME), interleaved with `...` stubs.
      const bodyRisk: { idx: number; line: string }[] = [];
      for (let k = cursor; k < bodyEnd && bodyRisk.length < MAX_BODY_RISK_LINES; k++) {
        if (!isBlank(orig[k]) && isRiskBodyLine(orig[k], mask[k])) bodyRisk.push({ idx: k, line: orig[k] });
      }
      const pyStub = (count: number): string =>
        `${bodyIndentStr}...  # ${ELL} ${count} ${count === 1 ? "line" : "lines"}`;
      const elidedCount = countNonBlank(orig, cursor, bodyEnd) - bodyRisk.length;
      if (bodyRisk.length > 0) {
        let prev = cursor;
        for (const r of bodyRisk) {
          const gap = countNonBlank(orig, prev, r.idx);
          if (gap > 0) {
            out.push(pyStub(gap));
            kept += 1;
          }
        ...
        const tailGap = countNonBlank(orig, prev, bodyEnd);
        if (tailGap > 0) {
          out.push(pyStub(tailGap));
          kept += 1;
        }
        elided += elidedCount;
      } else if (elidedCount > 0) {
        out.push(pyStub(elidedCount));
