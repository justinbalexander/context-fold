...
| `CONTEXTFOLD_PREFIX_STABLE` | off | `1` = prefix-stable folding: each epoch's substitutions commit as a **frozen layer** whose bytes never change again, and folding prefers the *oldest* unfrozen blocks — the context head stays byte-identical across turns, which is what keeps the provider's prompt cache warm (prefix caching is positional; a rewritten early block re-bills everything after it). Frozen folds no longer auto-unfold when pressure drops; `unfold {code}` remains the escape hatch. When the budget is unreachable the engine breaks the *oldest* layer as one deliberate, logged cache re-prefill and replans deeper. Persisted across resume byte-exactly. Default flips on once the e2e cache comparison wins ≥3 consecutive runs. |
